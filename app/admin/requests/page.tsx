...
export default function AdminRequestsPage() {
  ...
  const photosForPublish = (req: any) => {
    const photos: string[] = [];
    if (req.freePhotoUrl) photos.push(req.freePhotoUrl);
    const paidOk = req.paymentStatus === "confirmed";
    if (paidOk && Array.isArray(req.extraPhotoUrls)) {
      photos.push(...req.extraPhotoUrls.filter(Boolean));
    }
    return photos;
  };

  const handleApprove = async (req: any) => {
    const photos = photosForPublish(req);
    const extrasWaiting = (req.photoPack === "plus2" || req.photoPack === "plus5") && req.paymentStatus !== "confirmed";
    const note = extrasWaiting
      ? `Payment is not confirmed. Only the 1 free photo will go live for "${req.name}". Continue?`
      : `Approve and publish "${req.name}"?`;
  ...
  if (!isAdmin) {
    return <div className="min-h-screen flex items-center justify-center">Admin access only</div>;
  }
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          {editingId && (
            <div className="bg-white border rounded-2xl p-5 mb-8 space-y-3">
              ...
              <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Business name" className="w-full border rounded-xl px-4 py-3" />
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <button onClick={cancelEdit} className="bg-gray-100 text-gray-700 font-semibold px-5 py-2.5 rounded-xl">Cancel</button>
              </div>
            </div>
          )}
          ...
          {loading ? (
            ...
            <div className="bg-white border rounded-2xl p-5 text-gray-500 mb-8">No pending requests.</div>
          ) : (
              ...
              {pending.map((req) => (
                <div key={req.id} className="bg-white border rounded-2xl p-5">
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    {req.subcategory || req.category} · {req.district}
                  </p>
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    {req.freePhotoUrl && (
                      <div>
                        <p className="text-xs text-gray-500 mb-1">Free photo</p>
                        <img src={req.freePhotoUrl} alt="" className="w-20 h-20 object-cover rounded-lg border" />
                      </div>
                    )}
                    {(req.extraPhotoUrls || []).map((url: string, i: number) => (
                      <div key={url}>
                        <p className="text-xs text-gray-500 mb-1">Extra {i + 1}</p>
                        <img src={url} alt="" className="w-20 h-20 object-cover rounded-lg border" />
                      </div>
                    ))}
                    {req.paymentScreenshotUrl && (
                      <div>
                        <p className="text-xs text-gray-500 mb-1">Orange screenshot</p>
                        <a href={req.paymentScreenshotUrl} target="_blank" rel="noreferrer">
                          <img src={req.paymentScreenshotUrl} alt="Payment" className="w-20 h-20 object-cover rounded-lg border" />
                        </a>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-3 mt-4">
                    <button onClick={() => startEdit(req)} className="bg-gray-100 text-gray-800 font-semibold px-4 py-2 rounded-xl">Edit</button>
                    {(req.photoPack === "plus2" || req.photoPack === "plus5") && req.paymentStatus !== "confirmed" && (
                      <button onClick={() => handleConfirmPayment(req)} className="bg-amber-100 text-amber-800 font-semibold px-4 py-2 rounded-xl">
                        Confirm payment
                      </button>
                    )}
                    <button onClick={() => handleApprove(req)} className="bg-[#006B3F] text-white font-semibold px-4 py-2 rounded-xl">
                      Approve & Publish
                    </button>
                    <button onClick={() => handleReject(req)} className="bg-red-50 text-red-600 font-semibold px-4 py-2 rounded-xl border border-red-200">
                      Reject
                    </button>
                  </div>
                </div>
          ...
          <h2 className="text-lg font-semibold mb-3">Reviewed ({others.length})</h2>
          {others.length === 0 ? (
            <div className="bg-white border rounded-2xl p-5 text-gray-500">No reviewed requests yet.</div>
          ) : (
                ...
                <div key={req.id} className="bg-white border rounded-2xl p-4">
                  <div className="flex justify-between gap-3">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{req.category} · {req.district}</p>
                    </div>
                    <span className={`text-xs font-semibold px-2 py-1 rounded-full h-fit ${
                      req.status === "approved" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-700"
                    }`}>
                      {req.status}
                    </span>
                  </div>
